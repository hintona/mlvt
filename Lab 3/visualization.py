...
def scatter_hist( x, y, axisMain, axisX, axisY):
	'''
	Just a helper function to plot the scatter histogram 
	x (1darray), the data to go on the x axis
	y (1darray), the data to go on the y axis
	axisMain
	axisX
	axisY
	'''
	axisX.tick_params(axis="x", labelbottom=False)
	axisY.tick_params(axis="y", labelbottom=False)

	axisMain.scatter(x,y)

	binwidth = 5
	xymax = max(np.max(np.abs(x)), np.max(np.abs(y)))
	lim = (int(xymax/binwidth) + 1)

	bins = np.arange(0, lim + binwidth, binwidth)
	axisX.hist(x, bins=bins, orientation='horizontal')
	axisY.hist(y, bins=bins)

def scatter_hist_plot( data, headers, col_x, col_y,  title=None ):
	''' 
	ARGS:
	data (2darray), dataset that contains columns with the X and Y data to be plotted
	...
	axisMain = fig.add_axes(rect_scatter)
	axisMain.set_xlabel( headers[ col_x ] )
	axisMain.set_ylabel( headers[ col_y ] )
	axisX = fig.add_axes(rect_histx, sharex=axisMain)
	axisY = fig.add_axes(rect_histy, sharey=axisMain)
	
	scatter_hist(data[col_x], data[col_y], axisMain, axisY, axisX)
...
def main( argv ):
	...
	scatter_hist_plot( data, headers, col_x=5, col_y=7, title=title )
